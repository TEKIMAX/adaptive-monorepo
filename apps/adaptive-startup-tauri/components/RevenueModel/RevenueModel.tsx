...
import { Plus, Trash2, TrendingUp, DollarSign, Users, ChevronDown, Check, ArrowRight, Loader2, BarChart3, Calculator, Info, Presentation, Calendar, Sparkles, RefreshCw, Save, History, X, Download, AlertTriangle, Clock, FileText, ExternalLink, Send, Receipt } from 'lucide-react';
...
const RevenueModel: React.FC<RevenueModelProps> = ({
    ...
    const handleFollowUpChat = async (e: React.FormEvent) => {
        ...
        try {
            const response = await chatWithAIAnalyst(data, 'revenue', chatHistory, question, settings);
    ...
    const maxVal = Math.max(...projections.map(p => Math.max(p.revenue, p.costs)), 5000) * 1.15; // 15% headroom
    const graphHeight = 400;
    const graphWidth = 900;
    const padding = { top: 30, right: 50, bottom: 40, left: 70 };
    const drawingHeight = graphHeight - padding.top - padding.bottom;
    const drawingWidth = graphWidth - padding.left - padding.right;

    const getY = (val: number) => padding.top + drawingHeight - ((val / maxVal) * drawingHeight);
    const getX = (month: number) => padding.left + (((month - 1) / (Math.max(totalMonths - 1, 1))) * drawingWidth);
    ...
    // Calculate Grid Year Interval based on total duration to prevent overcrowding
    const yearInterval = forecastYears > 15 ? 5 : (forecastYears > 6 ? 2 : 1);

    // Calculate Polygons for Profit/Loss Areas with precise intersection
    let profitPaths: string[] = [];
    let lossPaths: string[] = [];

    // Helper to add path
    const addPath = (type: 'profit' | 'loss', x1: number, y1_top: number, y1_bottom: number, x2: number, y2_top: number, y2_bottom: number) => {
        // For profit: Revenue is top, Cost is bottom
        // For loss: Cost is top, Revenue is bottom
        const path = `M${x1},${y1_top} L${x2},${y2_top} L${x2},${y2_bottom} L${x1},${y1_bottom} Z`;
        if (type === 'profit') profitPaths.push(path);
    ...
    for (let i = 0; i < projections.length - 1; i++) {
        ...
        const x1 = getX(curr.month);
        const r1 = getY(curr.revenue);
        const c1 = getY(curr.costs);

        const x2 = getX(next.month);
        const r2 = getY(next.revenue);
        const c2 = getY(next.costs);

        const currDiff = curr.revenue - curr.costs; // + means profit
        const nextDiff = next.revenue - next.costs;

        // No intersection within this segment
        if ((currDiff >= 0 && nextDiff >= 0) || (currDiff < 0 && nextDiff < 0)) {
            if (currDiff >= 0) {
                // Profit Zone: Fill between Revenue (top visual) and Cost (bottom visual)
                // Note: SVG Y is inverted. Lower Y is higher visual.
                // Revenue Y is r1 (higher value -> lower Y). Cost Y is c1 (lower value -> higher Y).
                addPath('profit', x1, r1, c1, x2, r2, c2);
            } else {
                // Loss Zone: Fill between Cost (top visual) and Revenue (bottom visual)
                addPath('loss', x1, c1, r1, x2, c2, r2);
            }
        } else {
            // Intersection detected
            // Calculate t (0 to 1) where diff becomes 0
            const t = Math.abs(currDiff) / (Math.abs(currDiff) + Math.abs(nextDiff));
            const xi = x1 + t * (x2 - x1);
            const ri = r1 + t * (r2 - r1); // Revenue intersection Y
            const ci = c1 + t * (c2 - c1); // Cost intersection Y
            // Ideally ri === ci at intersection, but floating point might vary slightly, use average
            const yi = (ri + ci) / 2;

            // Segment 1 (from curr to intersection)
            if (currDiff >= 0) { // Started in profit
                addPath('profit', x1, r1, c1, xi, yi, yi);
            } else { // Started in loss
                addPath('loss', x1, c1, r1, xi, yi, yi);
            }

            // Segment 2 (from intersection to next)
            if (nextDiff >= 0) { // Ended in profit
                addPath('profit', xi, yi, yi, x2, r2, c2);
            } else { // Ended in loss
                addPath('loss', xi, yi, yi, x2, c2, r2);
            }
        }
    }

    // Line Paths
    const revenueLinePath = projections.map((p, i) => `${i === 0 ? 'M' : 'L'} ${getX(p.month)} ${getY(p.revenue)} `).join(' ');
    const costLinePath = projections.map((p, i) => `${i === 0 ? 'M' : 'L'} ${getX(p.month)} ${getY(p.costs)} `).join(' ');
    ...
    const handleGraphMouseMove = (e: React.MouseEvent<HTMLDivElement>) => {
        ...
        // Convert x to month
        const rawMonth = ((x - padding.left) / drawingWidth) * (totalMonths - 1) + 1;
        const month = Math.max(1, Math.min(totalMonths, Math.round(rawMonth)));
    ...
    const handleGenerateInsight = async () => {
        setIsAnalyzing(true);
        const insight = await analyzeRevenueModel(data, settings);
        ...
        try {
            if (exampleName === "NEW") {
                ...
                for (const s of data.revenueModel.revenueStreams) {
                    try { await deleteRevenueStream({ id: s.id as any }); } catch (e) { console.warn("Failed to delete stream", s.id, e); }
                }
                for (const c of data.revenueModel.costStructure) {
                    try { await deleteCost({ id: c.id as any }); } catch (e) { console.warn("Failed to delete cost", c.id, e); }
                }
    ...
    const confirmDeleteRevenue = () => {
        if (deleteRevenueTarget) {
            deleteRevenueStream({ id: deleteRevenueTarget as any });
    ...
    const confirmDeleteCost = () => {
        if (deleteCostTarget) {
            deleteCost({ id: deleteCostTarget as any });
            ...
            <div className="flex flex-col h-[calc(100vh-64px)] overflow-hidden relative">
                ...
                <div className={`w-full flex bg-[#F9F8F4] border-b border-stone-200 transition-all duration-300 ease-in-out ${isConfigExpanded ? 'h-[60%]' : 'h-[calc(100%-60px)]'}`}>
                    ...
                    <div className="flex-grow p-8 flex flex-col relative">
                        ...
                        <div
                            className="flex-grow w-full relative bg-white border border-stone-200 rounded-xl shadow-inner overflow-hidden"
                        ...
                        >
                            <svg viewBox={`0 0 ${graphWidth} ${graphHeight} `} className="w-full h-full overflow-visible">
                                ...
                                {[0, 0.25, 0.5, 0.75, 1].map((pct) => {
                                    const y = padding.top + (drawingHeight * pct);
                                    const val = maxVal * (1 - pct);
                                    return (
                                        <g key={pct}>
                                            <line x1={padding.left} y1={y} x2={graphWidth - padding.right} y2={y} stroke="#e7e5e4" strokeWidth="1" strokeDasharray="4 4" />
                                            <text x={padding.left - 12} y={y + 3} textAnchor="end" className="text-[10px] fill-stone-400 font-mono font-bold">
                                                {formatYLabel(val)}
                                            </text>
                                        </g>
                                    );
                                ...
                                {/* Grid Lines (Vertical - Years) - Optimized for long duration */}
                                {months.filter(m => m % 12 === 0 && (m / 12) % yearInterval === 0).map(m => (
                                    <g key={m}>
                                        <line x1={getX(m)} y1={padding.top} x2={getX(m)} y2={graphHeight - padding.bottom} stroke="#e7e5e4" strokeWidth="1" />
                                        <text x={getX(m)} y={graphHeight - 15} textAnchor="middle" className="text-[10px] fill-stone-400 font-bold font-sans uppercase">
                                            Year {m / 12}
                                        </text>
                                    </g>
                                ))}
                                ...
                                {/* Filled Areas */}
                                {lossPaths.map((d, i) => <path key={`loss - ${i} `} d={d} fill="url(#lossGrad)" stroke="none" />)}
                                {profitPaths.map((d, i) => <path key={`profit - ${i} `} d={d} fill="url(#profitGrad)" stroke="none" />)}

                                {/* Lines */}
                                <path d={costLinePath} fill="none" stroke="#ef4444" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" />
                                <path d={revenueLinePath} fill="none" stroke="#fbbf24" strokeWidth="4" strokeLinecap="round" strokeLinejoin="round" filter="drop-shadow(0px 2px 3px rgba(0,0,0,0.15))" />
                                ...
                                {breakEvenMonth && (
                                    <g>
                                        <circle
                                            ...
                                            cy={getY(projections[breakEvenMonth - 1].revenue)}
                                            r="6"
                                            fill="#10b981"
                                            stroke="white"
                                            strokeWidth="2"
                                ...
                                {hoveredMonth && (
                                    <g>
                                        <line
                                            ...
                                            strokeDasharray="2 2"
                                        />
                                        <circle cx={getX(hoveredMonth)} cy={getY(projections[hoveredMonth - 1].revenue)} r="4" fill="#fbbf24" stroke="white" strokeWidth="2" />
                                        <circle cx={getX(hoveredMonth)} cy={getY(projections[hoveredMonth - 1].costs)} r="4" fill="#ef4444" stroke="white" strokeWidth="2" />
                                    </g>
                                )}
                            ...
                            {hoveredMonth && (
                                <div
                                    className="absolute z-20 bg-white/95 backdrop-blur shadow-xl rounded-lg p-4 border border-stone-200 text-xs pointer-events-none min-w-[180px]"
                                    style={{
                                        left: Math.min(graphWidth - 180, Math.max(10, getX(hoveredMonth) / graphWidth * 100)) + '%',
                                        top: '10%',
                                        transform: `translateX(${hoveredMonth > totalMonths / 2 ? '-110%' : '10%'})`
                                    }}
                                >
                                    <div className="font-bold text-stone-900 mb-2 border-b border-stone-100 pb-1">Month {hoveredMonth} (Year {Math.ceil(hoveredMonth / 12)})</div>
                                    <div className="grid grid-cols-2 gap-x-4 gap-y-1">
                                        <span className="text-stone-500">Revenue</span>
                                        <span className="text-right font-mono font-bold text-amber-500">${projections[hoveredMonth - 1].revenue.toLocaleString()}</span>

                                        <span className="text-stone-500">Costs</span>
                                        <span className="text-right font-mono font-bold text-red-500">${projections[hoveredMonth - 1].costs.toLocaleString()}</span>

                                        <span className="text-stone-500 pt-1 border-t border-stone-100 mt-1">Net</span>
                                        <span className={`text-right font-mono font-bold pt-1 border-t border-stone-100 mt-1 ${projections[hoveredMonth - 1].profit >= 0 ? 'text-emerald-600' : 'text-red-600'}`}>
                                            ${projections[hoveredMonth - 1].profit.toLocaleString()}
                                        </span>
                                    </div>
                                </div>
                            )}
                            ...
                            <div className="absolute bottom-4 left-4 flex gap-4 bg-white/90 px-4 py-2 rounded-full border border-stone-100 shadow-sm backdrop-blur-sm z-10">
                                ...
                                <div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest text-emerald-600">
                                    <div className="w-3 h-3 bg-emerald-500/30 rounded-sm"></div> Profit Zone
                                </div>
                            </div>
                        </div>
                    </div>
                            ...
                            <div className="flex flex-col md:flex-row gap-8 items-start border-b border-stone-800 pb-8">
                                ...
                                <div className="grid grid-cols-2 gap-x-12 gap-y-8 flex-grow">
                                    ...
                                    <div>
                                        ...
                                        <input
                                            type="number"
                                            value={revenueModel.startingUsers}
                                            onChange={(e) => updateSettings({ startingUsers: parseInt(e.target.value) || 0 })}
                                    ...
                                    <div>
                                        ...
                                        <input
                                            type="number"
                                            value={revenueModel.cac}
                                            onChange={(e) => updateSettings({ cac: parseFloat(e.target.value) || 0 })}
                                            className="w-full pb-2 border-b border-stone-700 focus:border-nobel-gold outline-none font-serif text-3xl font-medium text-white bg-transparent placeholder-stone-700 disabled:opacity-70 disabled:cursor-not-allowed"
                                            placeholder="0"
                                            disabled={!canEdit}
                                        />
                                    ...
                                    <div>
                                        ...
                                        <input
                                            ...
                                            onChange={(e) => updateSettings({ monthlyGrowthRate: parseFloat(e.target.value) || 0 })}
                                    ...
                                    <div>
                                        ...
                                        <input
                                            ...
                                            onChange={(e) => updateSettings({ churnRate: parseFloat(e.target.value) || 0 })}
                                                        ...
                                                        <div className="flex items-start justify-between gap-3">
                                                            ...
                                                            <div className="flex items-center gap-1 shrink-0 bg-stone-900/50 px-2 py-1 rounded border border-stone-700/50">
                                                                ...
                                                                <input
                                                                    type="number"
                                                                    value={stream.price}
                                                                    onChange={(e) => updateRevenueItem(stream.id, 'price', parseFloat(e.target.value))}
                                                        ...
                                                        <div className="flex items-start justify-between gap-3">
                                                            ...
                                                            <div className="flex items-center gap-1 shrink-0 bg-stone-900/50 px-2 py-1 rounded border border-stone-700/50">
                                                                ...
                                                                <input
                                                                    type="number"
                                                                    value={cost.amount}
                                                                    onChange={(e) => updateCostItem(cost.id, 'amount', parseFloat(e.target.value))}
