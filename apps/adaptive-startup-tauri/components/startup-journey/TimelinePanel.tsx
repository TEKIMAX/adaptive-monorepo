...
export const TimelinePanel: React.FC<TimelinePanelProps> = ({
    ...
    return (
        <div className="flex-1 w-full bg-nobel-cream relative flex flex-col h-full overflow-hidden">
            ...
            {sheetMilestones && (
                <div className="fixed inset-y-0 right-0 w-[450px] bg-white shadow-2xl z-50 transform transition-transform animate-in slide-in-from-right duration-300 border-l border-stone-200 flex flex-col">
                    <div className="p-6 border-b border-stone-100 flex justify-between items-center bg-[#F9F8F4]">
                        <div className="flex items-center gap-2">
                            ...
                            <h3 className="font-serif text-xl text-stone-900">Milestone Details</h3>
                        </div>
                        <button onClick={() => setSheetMilestones(null)} className="p-2 hover:bg-white rounded-full transition-colors text-stone-400 hover:text-stone-900"><X className="w-5 h-5" /></button>
                    </div>
                    ...
                    <div className="flex-grow overflow-y-auto p-6 bg-white">
                        <div className="space-y-10">
                            {sheetMilestones.sort((a, b) => b.date - a.date).map((m, idx) => {
                                ...
                                const borderClass = m.tractionType === 'Traction' ? 'border-emerald-200' : m.tractionType === 'Pivot' ? 'border-rose-200' : 'border-stone-100';
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <div className="flex justify-between items-start mb-3">
                                            <div>
                                                <h4 className="font-serif font-bold text-lg leading-tight text-stone-900 group-hover:text-nobel-gold transition-colors">
                                                    {m.title}
                                                </h4>
                                                <div className="flex items-center gap-2 mt-1.5 text-[10px] uppercase font-bold tracking-wider text-stone-400">
                                                    <Calendar className="w-3 h-3" />
                                                    {new Date(m.date).toLocaleDateString(undefined, { year: 'numeric', month: 'long', day: 'numeric' })}
                                                </div>
                                            </div>
                                            <button className="text-stone-300 opacity-0 group-hover:opacity-100 transition-opacity">
                                        ...
                                        <div className="flex flex-wrap gap-2 mb-4">
                                            <span className="inline-flex items-center gap-1 text-[10px] bg-stone-100 text-stone-600 px-2 py-0.5 rounded border border-stone-200 font-bold uppercase tracking-wider">
                                                <Flag className="w-3 h-3" /> {m.type}
                                            </span>
                                            <span className={`inline-flex items-center gap-1 text-[10px] px-2 py-0.5 rounded border font-bold uppercase tracking-wider ${getTractionColorClass(m.tractionType)}`}>
                                        ...
                                        {m.imageUrl && (
                                            <img src={m.imageUrl} alt="Asset" className="w-full h-32 object-contain bg-stone-50 rounded-lg mb-4 border border-stone-100" />
                                        )}
