...
const CanvasStack: React.FC<CanvasStackProps> = ({
  ...
  const handleGenerate = async (section: CanvasSection): Promise<string> => {
    ...
    try {
      const suggestion = await suggestCanvasSection(section, data, settings);
      return suggestion;
    } catch (e) {
      console.error(e);
      return "";
  ...
  return (
    <div className="min-h-screen flex flex-col text-stone-900 bg-[#F9F8F4] relative">
      ...
      <header className="px-6 py-3 bg-white/80 backdrop-blur-sm sticky top-0 z-30 flex items-center justify-between border-b border-stone-200">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="relative">
            <button
              onClick={() => onNavigate('LEARN_BMC')}
              className="flex items-center gap-2 px-4 py-2 text-xs font-bold uppercase tracking-wider bg-purple-600 border border-purple-600 rounded-full transition-colors text-white hover:bg-purple-700 shadow-md group mr-3"
            >
      ...
      <div className="relative h-64 w-full bg-stone-900 shrink-0">
        ...
        <div className="relative z-10 px-12 h-full flex flex-col justify-center max-w-7xl mx-auto w-full">
          <span className="text-nobel-gold font-medium tracking-wider uppercase text-xs mb-2 block">Strategy</span>
          <h2 className="text-4xl md:text-5xl font-serif text-white mb-2">Business Model Canvas</h2>
          <p className="text-gray-300 text-lg">Define your business model on a single page.</p>
        </div>
      </div>

      <div className="flex-grow p-8 md:p-12 relative z-10 pb-32">
