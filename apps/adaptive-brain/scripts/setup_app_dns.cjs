...
const main = async () => {
    ...
    // Use subdomainName if provided, otherwise use organizationName
    const rawSubdomain = subdomainName || organizationName;
    ...
    const sanitizedSubdomain = sanitizeSubdomain(rawSubdomain);
    const baseDomain = 'adaptivestartup.io';
    const fullDomain = `${sanitizedSubdomain}.${baseDomain}`;
    const pagesTarget = `${projectName}.pages.dev`;

    console.log(`Setting up DNS for deployed app...`);
    console.log(`Subdomain: ${sanitizedSubdomain}`);
    ...
    try {
        ...
        if (process.env.GITHUB_OUTPUT) {
            const fs = require('fs');
            fs.appendFileSync(process.env.GITHUB_OUTPUT, `app_custom_domain=${fullDomain}\n`);
        } else {
            console.log(`::set-output name=app_custom_domain::${fullDomain}`);
        }

        console.log(`\n✅ DNS setup complete for deployed app!`);
        console.log(`Your app will be available at: https://${fullDomain}`);
