...
const BottomUpSizing: React.FC<BottomUpSizingProps> = ({ data, allProjects, onUpdateProject, onUpdateCanvas, onSwitchProject, onNewProject, onNavigate, currentView, settings: aiSettings, allowedPages, permissions }) => {
    ...
    const handleSaveBottomUp = async (updates: Partial<typeof bottomUpSizing>) => {
        ...
        try {
            ...
            console.error("Failed to save bottom-up data:", error);
        }
    };
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        {isGenerating ? (
                            ...
                            <div className="relative group/tooltip">
                                <button onClick={handleGenerateReport} disabled={isDisabled} className={`px-5 py-2 rounded-full text-xs font-bold uppercase tracking-widest transition-colors flex items-center gap-2 shadow-md ${isDisabled ? 'bg-stone-300 text-stone-500 cursor-not-allowed' : 'bg-stone-900 text-white hover:bg-nobel-gold'}`}>
                ...
                <main className="flex-grow flex relative z-10 overflow-hidden">
                    ...
                    <div className={`${isReportOpen ? 'w-1/2' : 'w-full'} flex flex-col px-12 py-6 overflow-y-auto transition-all duration-300`}>
                        <div className="max-w-4xl mx-auto w-full h-full flex flex-col">
                            {/* Header Area */}
                            <div className="mt-8 mb-8 shrink-0 flex flex-col gap-4">
                    ...
                    >
                        ...
                        <div className="px-8 py-4 border-b border-stone-100 flex items-center justify-between bg-stone-50/50 shrink-0">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <h2 className="font-serif text-xl text-stone-900">Strategic Analysis</h2>
                            ...
                            {isGenerating ? (
                                ...
                                isEditing ? (
                                    ...
                                    <div onClick={() => { if (canEdit) setIsEditing(true) }} className="cursor-text animate-in fade-in slide-in-from-right-4 duration-500">
                                        <MarkdownRenderer content={bottomUpSizing.reportContent} />
                                    </div>
                                )
                            ) : (
                                <div className="flex flex-col items-center justify-center py-20 text-stone-400">
