                ...
                <div className="flex-1 overflow-hidden p-6 relative z-10">
                    <div className="max-w-[1600px] mx-auto h-full flex gap-6 justify-center">
                        ...
                        <div className="flex-1 flex flex-col h-full bg-white rounded-3xl border border-stone-200 shadow-xl overflow-hidden max-w-4xl">
                            {/* Header */}
                            <div className="p-4 border-b border-stone-100 bg-stone-50/80 backdrop-blur-xl flex gap-3 shrink-0 items-center">
                                <div className="relative flex-1">
                                    <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-stone-400 w-4 h-4" />
                                    <input
                                        type="text"
                                        placeholder="Search models..."
                                        className="w-full pl-9 pr-4 py-2.5 rounded-xl border border-stone-200 focus:border-stone-400 focus:ring-0 outline-none bg-white text-sm transition-all"
                                        value={searchQuery}
                                ...
                                <button
                                    ...
                                    className="px-4 py-2.5 bg-stone-900 text-white rounded-xl hover:bg-black transition-colors shadow-lg hover:shadow-xl active:scale-95 flex items-center gap-2 text-sm font-medium"
                                >
                            ...
                            {/* Column Headers */}
                            <div className="grid grid-cols-12 gap-4 px-6 py-3 bg-stone-100/50 font-semibold text-xs text-stone-500 uppercase tracking-wider shrink-0 border-b border-stone-200/50">
                                    ...
                                    <div className="divide-y divide-stone-50">
                                        {models.map((model, idx) => (
                                            <div
                                                ...
                                                className={`grid grid-cols-12 gap-4 px-6 py-4 items-center hover:bg-stone-50 transition-colors cursor-pointer group ${selectedModel?.model_id === model.model_id ? 'bg-stone-100 border-l-4 border-stone-900 pl-[20px]' : 'pl-6 border-l-4 border-transparent'}`}
                                                onClick={() => setSelectedModel(model)}
                                            >
                                                <div className="col-span-6">
                                                    <div className="font-bold text-stone-800 text-sm group-hover:text-black transition-colors">{model.name}</div>
                                                    <div className="text-[10px] uppercase tracking-wider font-semibold text-stone-400 mt-0.5">{model.provider_name}</div>
                                                </div>
                                                <div className="col-span-3 text-right font-mono text-sm text-stone-600 group-hover:text-stone-900">
                                    ...
                                    <>
                                        ...
                                        <div className="space-y-4">
                                            ...
                                            <div className="bg-[#2c2c2e] p-1 rounded-lg flex mb-2">
                                                <button
                                                    ...
                                                    className={`flex-1 py-1.5 text-[10px] font-bold uppercase tracking-wider rounded-md transition-all ${calculatorMode === 'simple' ? 'bg-[#4CD964] text-black shadow-sm' : 'text-[#888] hover:text-white'}`}
                                                >
                                                    Simple
                                                </button>
                                                <button
                                                    onClick={() => setCalculatorMode('traffic')}
                                                    className={`flex-1 py-1.5 text-[10px] font-bold uppercase tracking-wider rounded-md transition-all ${calculatorMode === 'traffic' ? 'bg-[#4CD964] text-black shadow-sm' : 'text-[#888] hover:text-white'}`}
                                                >
                                                    Traffic Model
                                                </button>
                                            </div>
                                            ...
                                            {calculatorMode === 'simple' ? (
                                                <>
                                                    ...
                                                    <div className="space-y-2">
                                                        ...
                                                        <div className="relative">
                                                            <input
                                                                ...
                                                                className="w-full bg-[#2c2c2e] text-white text-xl font-mono p-4 rounded-xl border border-transparent focus:border-[#4CD964] outline-none transition-all text-right"
                                                            />
                                                            <span className="absolute left-4 top-1/2 -translate-y-1/2 text-[#555] text-xs font-bold">IN</span>
                                                        </div>
                                                        <div className="text-right text-[#555] text-xs">{(inputTokens / 1000000).toFixed(2)}M</div>
                                                    </div>
                                                    ...
                                                    <div className="space-y-2">
                                                        ...
                                                        <div className="text-right text-[#555] text-xs">{(outputTokens / 1000000).toFixed(2)}M</div>
                                                    </div>
                                                </>
                                            ) : (
                                                        ...
                                                        <div className="space-y-1">
                                                            ...
                                                            <div className="relative">
                                                                <input
                                                                    ...
                                                                    className="w-full bg-[#2c2c2e] text-white text-sm font-mono p-2.5 rounded-lg border border-transparent focus:border-[#4CD964] outline-none text-right"
                                                                />
                                        ...
                                        <div className="mt-auto pt-6">
                                            <button
                                                ...
                                                className="w-full py-4 bg-[#FF9F0A] text-white rounded-2xl hover:bg-[#FFB340] active:scale-[0.98] transition-all font-bold text-lg shadow-lg flex items-center justify-center gap-2 group"
                                            >
