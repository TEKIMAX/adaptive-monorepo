...
const TeamOrganization: React.FC<TeamOrganizationProps> = ({
    ...
    const syncMembers = useAction(api.workos.syncOrganizationMembers);
    ...
    // Actions
    const addSeats = useAction(api.stripe.addSeats);
    ...
    const revokeInvite = useAction(api.invites.revokeInvite);
    const resendInvite = useAction(api.invites.resendInvite);
    const deleteTeamMember = useAction(api.invites.deleteTeamMember);
    ...
    return (
        <div className="min-h-screen flex flex-col bg-[#F9F8F4] bg-[radial-gradient(#e5e7eb_1px,transparent_1px)] [background-size:16px_16px] text-stone-900 font-sans overflow-hidden">
            {/* Header */}
            <header className="px-6 py-3 bg-white/80 backdrop-blur-sm sticky top-0 z-30 flex items-center justify-between border-b border-stone-200">
                ...
                <div className="flex items-center gap-4">
                    {data.businessStructure && (
                        <div className="hidden md:flex items-center gap-2 px-3 py-1.5 bg-stone-900 border border-stone-900 rounded-full animate-in fade-in slide-in-from-top-2 shadow-sm">
            ...
            <main className="flex-grow overflow-y-auto relative">
                ...
                <div className="relative h-64 w-full bg-stone-900 shrink-0">
                    ...
                    <div className="relative z-10 px-12 h-full flex flex-col justify-center max-w-7xl mx-auto w-full">
                        <span className="text-nobel-gold font-medium tracking-wider uppercase text-xs mb-2 block">Organization</span>
                        <h2 className="text-4xl md:text-5xl font-serif text-white mb-2">Team & Organization</h2>
                        <p className="text-gray-300 text-lg">Manage your team members and their roles.</p>
                    </div>
                </div>

                <div className={`max-w-7xl mx-auto px-12 py-8 transition-all duration-300`}>
                    ...
                    {/* Side Sheet */}
                    < div className={`fixed inset-0 z-40 transition-opacity duration-300 ${showStructureSheet ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}>
                        <div className="absolute inset-0 bg-black/20 backdrop-blur-sm" onClick={() => setShowStructureSheet(false)} />
                        <div className={`fixed top-0 right-0 h-full w-[400px] bg-white shadow-2xl border-l border-stone-200 transform transition-transform duration-300 z-40 overflow-y-auto ${showStructureSheet ? 'translate-x-0' : 'translate-x-full'}`}>
                            <div className="p-6">
                                <div className="flex justify-between items-center mb-6">
                                    <h2 className="font-serif text-xl text-stone-900">Organization Details</h2>
                                    <button onClick={() => setShowStructureSheet(false)} className="text-stone-400 hover:text-stone-900">
                                        <X className="w-5 h-5" />
                                    </button>
                                </div>
                                ...
                                <div className="space-y-4">
                                    ...
                                    >
                                        <div className="space-y-2">
                                            {LEGAL_DOCS_LIST.map(doc => (
                                                <div key={doc} className="flex items-center justify-between p-3 bg-stone-50 rounded-lg border border-stone-100">
                                    ...
                                    >
                                        <div className="space-y-3">
                                            {GOV_CONTRACTING_LIST.map(item => (
                                                <div key={item} className="p-3 bg-stone-50 rounded-lg border border-stone-100">
                                                    <div className="flex justify-between items-center mb-2">
                                                        <span className="text-sm font-medium text-stone-900">{item}</span>
                                                        <span className={`text-[10px] font-bold uppercase px-2 py-0.5 rounded-full ${orgDetails.govContracting[item] === 'Completed' ? 'bg-green-100 text-green-700' :
                                                            orgDetails.govContracting[item] === 'Pending' ? 'bg-amber-100 text-amber-700' :
                                                    ...
                                                    <CustomSelect
                                                        ...
                                                        options={[
                                                            { label: 'Not Started', value: 'Not Started', color: 'bg-stone-100 text-stone-500' },
                                    ...
                                    >
                                        <div className="space-y-2">
                                            {COMPLIANCE_LIST.map(item => <div key={item} className="flex items-center justify-between p-3 bg-stone-50 rounded-lg border border-stone-100">
                                                    ...
                                                    {['Pending', 'Completed', 'Not Started'].map((status) => (
                                                        <button
                                                            ...
                                                            className={`text-[10px] py-1 rounded border ${(orgDetails.compliance[item] as any) === status
                                                                ? 'bg-stone-900 text-white border-stone-900 px-3'
                                                                : 'bg-white text-stone-500 border-stone-200 px-2 hover:border-stone-400'
                                                                }`}
                                                        >
                                                            {status}
                                                        </button>
