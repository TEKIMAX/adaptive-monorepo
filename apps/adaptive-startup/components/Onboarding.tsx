            ...
            <div className="flex-grow flex flex-col items-center p-6 pb-20">
                ...
                {mode === 'dashboard' ? (
                    ...
                    <div className="w-full min-h-screen flex items-center justify-center">
                        {(() => {
                            // Show Simple Form ONLY if subscribed/trialing AND onboarding is completed.
                            // Otherwise, route them through the full Onboarding Flow (steps).
                            const isSubscribed = user?.subscriptionStatus === 'active' || user?.subscriptionStatus === 'trialing';
                            const showSimpleForm = isSubscribed && user?.onboardingCompleted;

                            if (showSimpleForm) {
                                // PAID USERS: Simple Form
                                return (
                                ...
                                // UNPAID/NEW USERS: Full Flow (includes payment)
                                return (
                                    <OnboardingFlow
                                        user={user}
                                        mode="create"
                                        onComplete={(name, hypothesis, foundingDate) => {
                                            onComplete(name || "New Venture", hypothesis || "", foundingDate);
                                        }}
                                    />
                                );
                            }
