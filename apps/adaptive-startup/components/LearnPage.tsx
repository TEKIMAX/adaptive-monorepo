...
export const LearnPage: React.FC<LearnPageProps> = ({ data, onNavigate }) => {
    ...
    const stopConversation = useCallback(() => {
        if (sessionRef.current) {
            try { sessionRef.current.close(); } catch (e) { console.debug("Session cleanup:", e); }
    ...
    const startConversation = async () => {
        ...
        try {
            ...
            if (!apiKey) throw new Error("Authentication failed: API Key missing.");
                    ...
                    onmessage: async (message: any) => {
                        ...
                        if (message.toolCall) {
                            for (const fc of message.toolCall.functionCalls) {
                                ...
                                })).catch(err => console.error("Consultation Tool Error:", err));
                            }
                        }
                    },
                    onerror: (e) => {
                        console.error("Consultation Sync Error:", e);
