...
export const TABS: { id: ViewState; label: string; icon: any; children?: { id: ViewState; label: string }[] }[] = [
    ...
    },
    {
        ...
        children: [
            ...
            { id: 'IDEATION', label: 'Ideation & Whiteboard' },
    ...
    },
    {
        ...
        icon: DollarSign,
        children: [
            { id: 'STRIPE_DASHBOARD', label: 'Payouts & Invoices' },
        ]
    },
    {
        id: 'AI_ASSISTANT',
...
const TabNavigation: React.FC<TabNavigationProps> = ({ currentView, onNavigate, allowedPages, currentUserRole, projectFeatures, mode = 'light' }) => {
    ...
    const filteredTabs = visibleTabs.filter(tab => {
        ...
        // 2. Check Entitlements (Explorer Restrictions)
        // If the user's role is NOT Founder/Admin (i.e. they are Explorer/Trial), restrict tabs.
        // We assume 'currentUserRole' implies 'Founder' tier if set to 'Founder'.
        // However, it's safer to pass entitlements. But since we don't have them passed here,
        // we can infer from "currentUserRole". If it's missing or not Founder/Admin, treat as restricted.
        // WAIT: currentUserRole comes from the Project Member role, not Subscription Tier.
        // We should probably rely on a passed prop or useEntitlements hook if available.
        // Given existing props, let's see where it's called.
        // App.tsx likely calls this.

        // Let's assume for now if they are NOT a 'Founder' or 'Admin' in the *project*, they might still be restricted 
        // by their *subscription* if they are the owner.
        // This component doesn't seem to have full context.
        // Ideally we should update the caller (App.tsx) to filter `allowedPages`.
        // BUT, we can hardcode logic here if we had the subscription tier.

        // Actually, the user asked to disable buttons.
        // Let's modify the filtering logic in `App.tsx` instead or pass entitlements here.
        // Modifying `TabNavigation` to consume `useEntitlements` is safer.
        return true;
    });
    ...
    return (
        <div className="relative flex items-center gap-3" ref={menuRef}>
            ...
            {isOpen && (
                <div className="absolute top-full left-0 mt-2 w-72 bg-white border border-stone-200 rounded-xl shadow-xl overflow-hidden z-50 py-2 max-h-[80vh] overflow-y-auto">
                        ...
                        {filteredTabs.map(tab => {
                                                ...
                                                {tab.children.map(child => {
                                                    ...
                                                    return (
                                                        <button
                                                            key={child.id}
                                                            onClick={() => {
                                                                if (showTrialBadges && !isChildAllowed) {
                                                        ...
                                                        >
                                                            ...
                                                            <div className="flex items-center gap-2">
                                                                {child.id === 'ADAPTIVE_LEARNING' ? (
                                                                    ...
                                                                    <span className={`text-[10px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded-full ${isChildAllowed
                                                                        ? 'bg-stone-100 text-stone-500'
                                                                        : 'bg-nobel-gold text-white'}`}>
                                                                        {isChildAllowed ? 'Free' : 'Pro'}
                                                                    </span>
                                                                )}
                            ...
                            return (
                                ...
                                >
                                    ...
                                    {showTrialBadges && (
                                        <div className="flex items-center gap-2">
                                            <span className={`text-[10px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded-full ${isAllowed
                                                ? 'bg-stone-100 text-stone-500'
                                                : 'bg-nobel-gold text-white'}`}>
                                                {isAllowed ? 'Free' : 'Pro'}
                                            </span>
                                            {!isAllowed && <Lock className="w-3 h-3 text-stone-400" />}
                                        </div>
                                    )}
                                </button>
                            );
