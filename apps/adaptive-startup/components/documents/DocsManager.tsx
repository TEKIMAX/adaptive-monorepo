...
import { ArrowLeft, Plus, FileText, MoreVertical, Search, Folder, FolderOpen, ChevronRight, ChevronDown, PanelLeftClose, PanelLeftOpen, Trash2, Edit2, Check, X, Lock, Unlock, Share2, UserPlus, ShieldCheck } from 'lucide-react';
...
export const DocsManager: React.FC<DocsManagerProps> = ({
    ...
    const items: FileSystemItem[] = useMemo(() => {
        ...
        folders.forEach(f => processed.push({
            id: f._id,
            type: 'folder',
            title: f.name,
        ...
        documents.forEach(d => processed.push({
            id: d._id,
            type: 'doc',
        ...
        if (draggedId) {
            ...
            if (draggedItem?.type === 'folder') {
                ...
                while (current && current.parentId) {
                    if (current.parentId === draggedId) { isDescendant = true; break; }
                    current = items.find(i => i.id === current.parentId);
                }
                // Also check direct parent logic if 'id' is a child item
                const targetItem = items.find(i => i.id === id);
                if (targetItem?.parentId === draggedId) isDescendant = true;
    ...
    return (
        <div className="flex h-screen w-full bg-nobel-cream canvas-pattern font-sans text-nobel-dark animate-fade-in-up overflow-hidden flex-col" style={{ backgroundSize: '24px 24px' }}>
            ...
            {/* Header */}
            <header className="px-6 py-3 bg-white/80 backdrop-blur-sm sticky top-0 z-30 flex items-center justify-between border-b border-stone-200 print:hidden shrink-0">
                ...
                <div className="flex-grow flex flex-col h-full bg-nobel-cream canvas-pattern relative min-w-0 transition-all duration-300" style={{ backgroundSize: '24px 24px' }}>
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        */}
                                        ...
                                        <button
                                            ...
                                            className="p-2 text-stone-400 hover:bg-stone-100 hover:text-stone-600 rounded-full transition-colors order-1"
                    ...
                    {activeDoc && (
                        <ShareDialog
                            ...
                            docId={activeDoc.id as Id<"documents">}
                            docTitle={activeDoc.title}
                            orgId={(activeDoc as any).orgId || me?.orgIds?.[0]}
                            currentCollaborators={(activeDoc as any).collaborators || []}
                            isLocked={!!(activeDoc as any).isLocked}
                            creatorId={(activeDoc as any).creatorId}
                            me={me}
                        />
                    )}
                </div>
