...
const MiniEditor: React.FC<MiniEditorProps> = ({
    ...
    useEffect(() => {
        if (editor && content) {
            ...
            if (currentClean !== incomingClean && !editor.isFocused && !isApplyingAi.current) {
                console.log('MiniEditor: Syncing external content', { incomingCleanLength: incomingClean.length, currentCleanLength: currentClean.length });
    ...
    const handleGenerateClick = async () => {
        ...
        console.log('MiniEditor: Starting AI generation...');
        // Expect string return
        try {
            const generatedContent = await onGenerateAI();
            console.log('MiniEditor: AI generation complete', { length: generatedContent?.length });
            ...
            if (typeof generatedContent === 'string' && generatedContent.trim() && editor) {
                ...
                console.log('MiniEditor: Applying AI update to state');
                onUpdate(markdown);
                ...
                console.warn('MiniEditor: AI returned empty or invalid content');
            }
        } catch (error) {
            console.error('MiniEditor: AI generation failed', error);
            isApplyingAi.current = false;
        }
    };
    ...
    return (
        <div className={`transition-colors flex flex-col ${disabled ? '' : 'bg-white border border-gray-300 rounded-md focus-within:ring-2 focus-within:ring-nobel-gold focus-within:border-transparent'} ${className}`}>
            {/* Fixed Toolbar - Always render to maintain DOM stability, hide via CSS when disabled */}
            <div
                className={`items-center gap-1 p-2 border-b border-gray-100 bg-gray-50/50 ${disabled ? 'hidden' : 'flex'}`}
            >
                <button
