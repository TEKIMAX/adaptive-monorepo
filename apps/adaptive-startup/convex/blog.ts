...
export const listCategories = query({
    ...
    handler: async (ctx, args) => {
        ...
        let query = ctx.db.query("blog_categories");

        if (args.projectId) {
            return await query.withIndex("by_project", q => q.eq("projectId", args.projectId)).collect();
        }

        if (args.orgId) {
            return await query.withIndex("by_org", q => q.eq("orgId", args.orgId)).collect();
        }
