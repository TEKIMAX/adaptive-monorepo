...
export const sendInvite = action({
    ...
    handler: async (ctx, args) => {
        ...
        const orgRole = userRoles.find(r => r.orgId === targetOrgId);
        ...
        // Check Seat Limits
        const totalSeats = user.seatCount || 1; // Default 1 seat

        // Count current members + pending invites
        // Ideally we check WorkOS, but we can also check our local team_members for speed if synced.
        // However, WorkOS is the source of truth for "active in org".
        // Also need to count pending invites if we charge for them? Yes usually.
        // WorkOS listOrganizationMemberships gives active members.
        // WorkOS listInvitations gives pending invites.

        const existingMembers = await getWorkOS().userManagement.listOrganizationMemberships({
            organizationId: targetOrgId,
            limit: 100 // Assume small teams for now
        });

        const pendingInvites = await getWorkOS().userManagement.listInvitations({
            organizationId: targetOrgId,
            limit: 100
        });

        // Filter out the current user if they are owner? No, owner takes a seat.
        // Used seats = Members + Pending Invites
        const usedSeats = existingMembers.data.length + pendingInvites.data.length;

        if (usedSeats >= totalSeats) {
            throw new Error(`SEAT_LIMIT_EXCEEDED: You have used ${usedSeats} of ${totalSeats} seats (Active: ${existingMembers.data.length}, Pending: ${pendingInvites.data.length}). Upgrade required.`);
        }
...
export const revokeInvite = action({
    ...
    handler: async (ctx, args) => {
        ...
        try {
            ...
            console.error("Failed to revoke WorkOS invite:", error);
        }
            ...
            if (member.email && member.orgId) {
                try {
                    ...
                    if (membershipIdToDelete) {
                        ...
                        console.warn(`Could not find WorkOS membership for ${member.email} in org ${member.orgId}`);
                    }

                } catch (error) {
                    console.error("Failed to remove from WorkOS:", error);
                }
            }
...
export const resendInvite = action({
    ...
    handler: async (ctx, args) => {
        ...
        try {
            ...
            console.error("Failed to resend WorkOS invite:", error);
