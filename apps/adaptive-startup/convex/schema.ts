...
export default defineSchema({
  ...
  }).index("by_org", ["orgId"]).index("by_localId", ["localId"]).index("by_creator", ["userId"]).index("by_slug", ["slug"]),
  ...
  files: defineTable({
    projectId: v.id("projects"),
    orgId: v.string(),
    folderId: v.optional(v.union(v.id("folders"), v.null())), // null for root
    name: v.string(),
    title: v.optional(v.string()),
    description: v.optional(v.string()),
    tags: v.optional(v.array(v.object({ name: v.string(), color: v.string() }))),
    ...
    size: v.number(),
    createdAt: v.number(),
    initiativeId: v.optional(v.id("initiatives")), // Link to Initiative
  }).index("by_project_folder", ["projectId", "folderId"])
  ...
  legal_documents: defineTable({
    ...
    recipientId: v.optional(v.string()), // local or convex ID of team member
    status: v.string(), // Pending, Signed
    content: v.optional(v.string()),
    attachmentUrl: v.optional(v.string()), // Base64 or URL
    fields: v.optional(v.string()), // JSON array of LegalField
    createdAt: v.number(),
    signedAt: v.optional(v.number()),
  ...
  chats: defineTable({
    ...
    createdAt: v.number(),
  }).index("by_org", ["orgId"]).index("by_user", ["userId"]).index("by_project", ["projectId"]),
